...
- Fixed an error that made so that docker wouldn't create the databse required.


## Postgres Admin Panel
### Also there is access to a Postgres Admin Panel
- The endpoint is [http://localhost:8080/](http://localhost:8080)
- The credentials are: 
  - User: example@example.com
  - Pass: notpassword
- After that the server connection is:
  - Host: postgres
  - User: root
  - Pass: notroot
